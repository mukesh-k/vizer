...
class UIBuilder:
    ...
    def maximize_button(self, view, i, j):
        with vuetify.VBtn(tile=True, small=True, v_if=f'{view.id}_no_maximized',
...
class Quad(Base):
    ...
    def create_widget(self):
        ...
        with self.layout.viewport:
            with vuetify.VRow(v_if=f'{self._id}_no_maximized || {self.id}_max_row == 0', no_gutters=True,
                              classes="grow"):
            ...
            with vuetify.VRow(v_if=f'{self._id}_no_maximized || {self.id}_max_row == 1', no_gutters=True,
                              classes="grow"):
                with vuetify.VCol(v_if=f'{self._id}_no_maximized || {self.id}_max_col == 0'):
                    with vuetify.VContainer(classes="fill-height pa-0 ma-0"):
        ...
        with self.layout.button_bar:
            if not self._force_outer_slices:
                with vuetify.VCol(cols='auto'):
                    self._ui_builder.toggle_button(self, var='show_inner_slices', on_icon='mdi-border-outside',
                                                   off_icon='mdi-border-inside',
                                                   on_text='Show outer faces', off_text='Show inner slices')
            if self.opts.subsampling_factor > 1:
                with vuetify.VCol(cols='auto'):
                    self._ui_builder.toggle_button(self, var='full_res', off_icon='mdi-quality-high',
                                                   on_icon='mdi-quality-low',
    ...
    def _update_3d_slice_visibility(self):
        ...
        for axis in range(3):
            ...
            for side in range(2):
                simple.Show(self._outer_slices[axis * 2 + side], view) if not show_inner_slices else simple.Hide(
                    self._outer_slices[axis * 2 + side], view)
    ...
    def update_color_map(self):
        ...
        if self.meta.raw_config is not None and self.meta.raw_config.colormap is not None:
            ...
            drange = self.producer.GetDataInformation().GetArrayInformation(self.get_scalar_name(),
                                                                            vtk.vtkDataObject.FIELD_ASSOCIATION_POINTS).GetComponentRange(
                0)
